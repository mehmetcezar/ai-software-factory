...
$bdbelgeler=$row['belgepaths'];
  ...
  $result -> free_result();
    ...
    if ($kiralandiison != 1 && $yetkilionay==1 && $guncellemegonderildi==1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu kiralamaya ait tahsilat güncellemesi yapılamaz. Ana menüye dönmek için tıklayınız. </z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
}
    
function parseDataToArray($data) {
    // ";" karakterine göre veriyi ayır ve bir dizi olarak döndür
    return explode(";", $data);
}
 ...
 $bdbelgelerarray=parseDataToArray($bdbelgeler);
...
$kaporabelgepaths = "";        
$basePath = "proje/mulk/{$projectId}/onaybektahsilat/{$kiralamakey}/";
...
$total_count = count($_FILES['kaporaBelge']['name']); // Yüklenen dosyaların sayısını al
    ...
    function isFileEmpty($file, $index) {
    // Dosya adı boş veya boyutu sıfırsa ya da yükleme sırasında hata oluşmuşsa
    if (empty($file['name'][$index]) || 
        $file['error'][$index] !== UPLOAD_ERR_OK || 
        $file['size'][$index] == 0) {
        return false; // Dosya boş
    }
    return true; // Dosya boş değil
}
...
for ($i = 0; $i < $total_count; $i++) { 
    ...
    if(isFileEmpty($_FILES['kaporaBelge'], $i) )
    {
    
    if($tahsilatturu[$i]=="KİRA")
    {
       $isimek="kiraBelge"; 
    }
    ...
    if($tahsilatturu[$i]=="DEPOZİTO")
    {
       $isimek="depozitoBelge"; 
    }
    if($tahsilatturu[$i]=="KAPORA")
    {
       $isimek="kaporaBelge"; 
    }
    if($tahsilatturu[$i]=="SU")
    {
       $isimek="suBelge"; 
    }
    if($tahsilatturu[$i]=="HİZMET")
    {
       $isimek="hizmetBelge"; 
    }
     if($tahsilatturu[$i]=="İNTERNET")
    {
       $isimek="internetBelge"; 
    }
         if($tahsilatturu[$i]=="KOMİSYON")
    {
       $isimek="komisyonBelge"; 
    }
    ...
    else
    {
       ...
       $kaporabelgepaths .= $bdbelgelerarray[$i] . ";"; 
    }
}  
   
     function convertArrayToStringrates($array,$PB) {
     $rate="";
          $j=0;
    ...
    return rtrim($rate, ";");     
         ...
         function convertArrayToStringtorates($array,$PB) {
     $rate="";
             $j=0;
     for ($i = 0; $i < count($PB); $i++) {    
   ...
   function convertArrayToString($array) {
    ...
    $kaporaTlRatedb=convertArrayToStringrates($kaporaTlRate,$kaporaPB);

    $kaporaToTlRatedb=convertArrayToStringtorates($kaporaToTlRate,$kaporaPB);
  ...
  // $sql = "INSERT INTO tahsilat (id, date, tahtarihi, mulkno, yapino, kiralamakey, tahsilatturu, miktar, parabirimi, tahsilattlrate, tahsilattotlrate, belgepaths, tahsilatnot, yetkilionay, company_id) VALUES ('', '$date','$tahsilattarihidb','$mulkno', '$yapino','$kaporaid','$tahsilatturudb','$kaporaSterlingdb','$kaporaPBdb','$kaporaTlRatedb','$kaporaToTlRatedb','$kaporabelgepaths','$tahsilatnotdb','$yetkilionay', '{$_SESSION['company_id']}')";
    ...
    echo $kaporaTlRatedb;
    echo "<br>";
    echo $kaporaToTlRatedb;
    exit();
    */
    
    $guncellemegonderildi=0;// güncelleme yok ise 0 olur.
    $sql = "UPDATE tahsilat SET gundate = '$date',gunusername = '$uname',tahtarihi = '$tahsilattarihidb',tahsilatturu = '$tahsilatturudb',miktar = '$kaporatahMiktardb',stgmiktar = '$kaporaSterlingdb',parabirimi = '$kaporaPBdb',tahsilattlrate = '$kaporaTlRatedb',tahsilattotlrate = '$kaporaToTlRatedb',belgepaths = '$kaporabelgepaths',tahsilatnot = '$tahsilatnotdb',yetkilionay = '$yetkilionay', guncellemegonderildi = '$guncellemegonderildi' WHERE id = '$tahsilatid'";
