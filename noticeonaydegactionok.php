...
//echo "Connected successfully";
  ...
  if ($kiraison == 1) {
   ...
   exit();
          ...
          if ($result = $conn -> query("SELECT noticetarihi FROM mulkkayit where id='$mulkno'")) {
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {

  
      $noticetarihireq=$row['noticetarihi'];
  ...
  $result -> free_result();
    ...
    if($varmikontrol)
    {
    $sql = "UPDATE noticereddet SET noticereddet.isdeleted=1 where noticereddet.kirakey='$kirakaporasonkey'"; 
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $mulkno= mysqli_real_escape_string($conn, $mulkno); 
      ...
      $noticeverildimi=2; // OnAYLANDI 2 OLUR 
      $status="NOTICEVERILDI";
      $sql = "UPDATE mulkkayit SET noticetarihi='$noticetarihireq', noticeverildimi='$noticeverildimi', status='$status' where id='$mulkno'"; 
    ...
    if ($conn->query($sql) === TRUE) {
             ...
             $sql = "UPDATE kiralamakayit SET noticetarihi='$noticetarihireq', noticeverildimi='$noticeverildimi', status='$status' where kiralamakey='$kirakaporasonkey'";
         ...
         if ($conn->query($sql) === TRUE) {
           ...
           echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait notice tarihi onayı başarıyla tamamlanmıştır.  Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
