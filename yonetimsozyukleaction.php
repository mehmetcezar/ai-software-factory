...
<?php 
    ...
    if($_FILES['yonetimsozimzali']['name']!=NULL) {
        $yonetimsozimzali = $_FILES['yonetimsozimzali'];
    }
    else
    {
      Notdevam();
...
$yonetimsozimzalipaths="";
    
    
    $basePath = "proje/mulk/{$projectId}/yonetim/imzalisoz/";
createDirectory($basePath);//KLASÖR İLK KEZ BURDA OLUŞTURULDU        
      
$i=randomNumbersInRange(1,99999);
       // Dosya adını ve uzantısını al
        $originalFileName = $_FILES['yonetimsozimzali']['name'];
        $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
$newFilePath = $basePath.$projectId."_yonetimsozimzali_".$currentDate.$i. "." . $fileExtension;
//DBye yazmak için değişkene dosya pathını kopyala
$yonetimsozimzalipaths=$yonetimsozimzalipaths.$newFilePath;
         if(move_uploaded_file($yonetimsozimzali['tmp_name'], $newFilePath)) {
        //echo "OK2"; 
...
//echo "Connected successfully";
      ...
      $sql = "UPDATE kiralamakayit SET yonetimsozimzalipath='$yonetimsozimzalipaths' where kiralamakayit.kiralamakey='$kirakaporasonkey'";
