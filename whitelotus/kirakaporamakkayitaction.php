...
<?php 
    ...
    if($_FILES['kaporamak']['name']!=NULL) {
        $kaporamak = $_FILES['kaporamak'];
    }
    else
    {
      Notdevam();
...
$kaporamakpaths="";
    ...
    $basePath = "proje/mulk/{$projectId}/kapora/mak/";
createDirectory($basePath);//KLASÖR İLK KEZ BURDA OLUŞTURULDU        
      
$i=randomNumbersInRange(1,99999);
       // Dosya adını ve uzantısını al
        $originalFileName = $_FILES['kaporamak']['name'];
        $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
$newFilePath = $basePath.$projectId."_kiramak_".$currentDate.$i. "." . $fileExtension;
//DBye yazmak için değişkene dosya pathını kopyala
$kaporamakpaths=$kaporamakpaths.$newFilePath;
         if(move_uploaded_file($kaporamak['tmp_name'], $newFilePath)) {
        //echo "OK2"; 
...
//echo "Connected successfully";
      ...
      $sql = "UPDATE kirakaporakayit SET kaporamakpath='$kaporamakpaths' where kirakaporakayit.kirakey='$buserid'";
 ...
 if ($conn->query($sql) === TRUE) {
     ...
     echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulknoid.' numaralı mülkün makbuzu başarıyla yüklenmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>'; 
