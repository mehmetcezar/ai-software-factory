...
<script src="js/bootstrap.min.js"></script>
 ...
 <script>
        ...
        window.location.href = "https://whitelotustest.online/kaporaiptalsureuzat.php";
    }
    </script>
...
//echo "Connected successfully";
  ...
  $result -> free_result();
    ...
    if($varmikontrol){
    $sql = "UPDATE kaporasurereddet SET kaporasurereddet.isdeleted=1  where kaporasurereddet.isdeleted !=1 AND kaporasurereddet.kirakey='$kirakaporasonkey'";
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $tarih = date('Y-m-d');
         ...
         $i=0;
$sql = "UPDATE mulkkayit SET kirakaporaeklendi='$i', kirakaporasontarih='', kaporaonayinda='$i', kaporasureonayinda='$i', kaporasureuzatmadate='',cronjobiptalkapora=0, status='KAPORAIPTAL' where mulkkayit.kirakaporasonkey='$kirakaporasonkey'";
    
    if ($conn->query($sql) === TRUE) {
   

     $status="GUNGECTIVEDUSTU";       
 $sql2 = "UPDATE kirakaporakayit SET kirakaporakayit.status='$status', kirakaporakayit.isdeleted=1,yetkilikaporaiptalaciklama='$aciklama' where kirakaporakayit.kirakey='$kirakaporasonkey'";
     ...
     if ($conn->query($sql) === TRUE) {
            ...
            echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait kapora iptal edilmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
         }
         else 
{
  echo "Error: " . $sql . "<br>" . $conn->error;
