...
//echo "Connected successfully";
                 ...
                 if ($result = $conn -> query("SELECT kaporakirabelgeektahsil,kaporamiktari,topalkaporamiktari,eskikaporamiktari FROM kirakaporakayit where kirakaporakayit.isdeleted !=1 AND kirakaporakayit.kirakey='$kirakaporasonkey'")) {
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {

     $kaporabelgepaths=$row['kaporakirabelgeektahsil'];
     $kaporamiktari=$row['kaporamiktari'];
      $topalkaporamiktari=$row['topalkaporamiktari'];
      $eskikaporamiktari=$row['eskikaporamiktari'];
}
  ...
  $result -> free_result();
    ...
    if($_SESSION['kaporamiktari']==$eskikaporamiktari) {
    echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Kapora ek tahsilatı işleminiz zaten gerçekleştirilmişti.  Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();   
       
   }
    
   if($kaporatahMiktar[0]>$topalkaporamiktari-$kaporamiktari) {
    echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Kapora ve tahsilat miktarına müdahale edilmiştir. İşleminiz gerçekleştirilemiyor. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();   
       
   }
    
    $kaporatahMiktar[0]=$kaporatahMiktar[0]+$kaporamiktari;
    $kaporatahMiktarxx=$kaporatahMiktar[0];
...
//$kaporabelgepaths = "";        
$basePath = "proje/mulk/{$projectId}/kapora/ektahsilat/";
...
for ($i = 0; $i < $total_count; $i++) {
   ...
   $kaporakirabelgeektahsilyapildimi=1; //1 ise evet. 0 ise hayır.
   $sql = "UPDATE kirakaporakayit SET kaporakirabelgeektahsilyapildimi = '$kaporakirabelgeektahsilyapildimi', kaporakirabelgeektahsil = '$kaporabelgepaths', kaporamiktari = '$kaporatahMiktarxx', eskikaporamiktari='$kaporamiktari' WHERE kirakaporakayit.kirakey = '$kirakaporasonkey'";
...
/*
   ...
   $_SESSION['kaporamiktari']=$kaporamiktari;
 
            echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Kapora tahsilatı önceki tahsilata eklenmiştir. Kapora ekranından görebilirsiniz. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
