...
//echo "Connected successfully";
  ...
  if ($kiraison == 1) {
   ...
   exit();
     ...
     $varmikontrol=0;
      if ($result = $conn -> query("SELECT * FROM zararziyanreddet where zararziyanreddet.isdeleted !=1 AND zararziyanreddet.kirakey='$kirakaporasonkey'")) {
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {
  ...
  $result -> free_result();
    ...
    if($varmikontrol){
    $sql = "UPDATE zararziyanreddet SET zararziyanreddet.isdeleted=1  where zararziyanreddet.isdeleted !=1 AND zararziyanreddet.kirakey='$kirakaporasonkey'";
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $tarih = date('Y-m-d');
     ...
     $sql = "INSERT INTO zararziyanreddet (id, username, date, mulkno, kirakey, yetkiliaciklama ) VALUES ('', '$uname','$date','$mulkno','$kirakaporasonkey','$aciklama')";
    
    if ($conn->query($sql) === TRUE) {
   

      $zararziyangirildimi=3;   //reddedildi
        //$kaporasureuzatmadate="0000-00-00";
        $status="ZARARZIYANRED";
         $sql = "UPDATE mulkkayit SET zararziyangirildimi='$zararziyangirildimi', status='$status' where id='$mulkno'"; 
     ...
     if ($conn->query($sql) === TRUE) {
        ...
        $status="ZARARZIYANRED";
         ...
         $sql = "UPDATE kiralamakayit SET zararziyangirildimi='$zararziyangirildimi', status='$status' where kiralamakey='$kirakaporasonkey'";
        ...
        if ($conn->query($sql) === TRUE) {
                   ...
                   echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait Zarar-Ziyan Girişi iptal edilmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
