...
<script src="js/bootstrap.min.js"></script>
 ...
 <script>
        ...
        window.location.href = "https://whitelotustest.online/noticeonay.php";
    }
    </script>
...
//echo "Connected successfully";
  ...
  if ($kiraison == 1) {
   ...
   exit();
     ...
     $varmikontrol=0;
      if ($result = $conn -> query("SELECT * FROM noticereddet where noticereddet.isdeleted !=1 AND noticereddet.kirakey='$kirakaporasonkey'")) {
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {
  ...
  $result -> free_result();
    ...
    if($varmikontrol){
    $sql = "UPDATE noticereddet SET noticereddet.isdeleted=1  where noticereddet.isdeleted !=1 AND noticereddet.kirakey='$kirakaporasonkey'";
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $tarih = date('Y-m-d');
     ...
     $sql = "INSERT INTO noticereddet (id, username, date, mulkno, kirakey, yetkiliaciklama ) VALUES ('', '$uname','$date','$mulkno','$kirakaporasonkey','$aciklama')";
    
    if ($conn->query($sql) === TRUE) {
   

      $noticeverildimi=3;   //reddedildi
        //$kaporasureuzatmadate="0000-00-00";
        $status="NOTICERED";
         $sql = "UPDATE mulkkayit SET noticeverildimi='$noticeverildimi', status='$status' where id='$mulkno'"; 
     ...
     if ($conn->query($sql) === TRUE) {
        ...
        $status="NOTICERED";
         ...
         $sql = "UPDATE kiralamakayit SET noticeverildimi='$noticeverildimi', status='$status' where kiralamakey='$kirakaporasonkey'";
        ...
        if ($conn->query($sql) === TRUE) {
                   ...
                   echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait Notice Tarihi iptal edilmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
