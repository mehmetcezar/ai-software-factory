...
//echo "Connected successfully";
  ...
  $result -> free_result();
    ...
    $sql = "DELETE FROM kiralamaguncellemedif where kiralamaguncellemedif.isdeleted !=1 AND kiralamaguncellemedif.kirakey='$kirakaporasonkey'";
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $tarih = date('Y-m-d');
     ...
     $sql = "INSERT INTO kiralamaguncellemedif (id, username, date, mulkno, kirakey, yetkiliaciklama ) VALUES ('', '$uname','$date','$mulkno','$kirakaporasonkey','$aciklama')";
    
    if ($conn->query($sql) === TRUE) {
   $status="KIRAGUNCELLE";

      $kiralamaguncellemegonderildi=1;   
     ...
     if ($conn->query($sql) === TRUE) {
         ...
         $sql = "UPDATE kiralamakayit SET status=' $status' where id='$mulkno'"; 
         ...
         if ($conn->query($sql) === TRUE) {
            ...
            echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait kiralama süreci başarıyla güncellemeye gönderilmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
         }
         else 
{
  echo "Error: " . $sql . "<br>" . $conn->error;
