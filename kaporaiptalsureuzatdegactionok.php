...
<?php 
    ...
    if($_POST['kaporauzatmatarihi']==NULL){
        Notdevam();
    }
    else
    {
      $kaporauzatmatarihi=$_POST['kaporauzatmatarihi'];
        $kaporauzatmatarihi=trim($kaporauzatmatarihi);
    } 
...
//echo "Connected successfully";
  ...
  if ($cronjobiptalkapora != 1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Kaporanın süresi henüz dolmamıştır. Yetkili işlemi yapılmaz. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
}   
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $mulkno= mysqli_real_escape_string($conn, $mulkno); 
      ...
      $cronjobiptalkaporax=0;
      $status="KAPORASUREONAYLANDI";
      $sql = "UPDATE mulkkayit SET kirakaporasontarih='$kaporauzatmatarihi', kaporasureonayinda='$kaporasureonayinda', cronjobiptalkapora='$cronjobiptalkaporax', status='$status' where id='$mulkno'"; 
    ...
    if ($conn->query($sql) === TRUE) {
             ...
             $sql = "UPDATE kirakaporakayit SET kaporasontarih='$kaporauzatmatarihi' where kirakey='$kirakaporasonkey'";
