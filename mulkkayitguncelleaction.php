...
<?php
 ...
 if ($mulkidsession != $mulkid) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Kayıt için başlatılan mülk numarası ile formda gönderilenmülk numarası farklıdır. Lütfen yeniden deneyiniz. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';
   exit();
}   
...
//echo "Connected successfully";
  ...
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {
    ...
    $esyalistesidb = $row['esyalistesi'];
    $ekbelgeyukledb = $row['ekbelgeyukle'];
  ...
  $result -> free_result();
   ...
   if ($mulkison == 1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu mülk silinmiştir. Güncelle işlemi yapılamaz.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
 ...
 $sql = "UPDATE mulkkayit SET adsoyad='$adsoyad', iletisim1='$iletisim1',iletisim2='$iletisim2',kimlikno='$kimlikno',email='$email',uyruk='$uyruk',msozelnot='$msozelnot',kirabedeli='$kirabedeli',kirabedeliparabirimi='$kirabedeliparabirimi', guncelaidat='$guncelaidat',aidatparabirimi='$aidatparabirimi', maxkirainoran='$maxkirainoran',susahiplik='$susahiplik',elektriksahiplik='$elektriksahiplik',mulkozelnot='$mulkozelnot', aidatsurekliligi='$aidatsurekliligi', gunusername='$uname',gundate='$date',kiralamadurumu='$kiralamadurumu' WHERE id='$mulkid'"; 
...
// Dosyanın boş olup olmadığını kontrol etmek
if(isset($_FILES['aktifkocan']['name'][0]) && $_FILES['aktifkocan']['name'][0] != "") {     
    
     if (unlink($aktifkocandb)) {
   // echo "Dosya başarıyla silindi.";
} else {
    echo "Dosya silinirken bir hata oluştu.";exit();
        //exit();
} 
...
$sql = "UPDATE mulkkayit SET aktifkocan='$aktifkocanpaths' where id='$mulkyapino'"; 
...
// Dosyanın boş olup olmadığını kontrol etmek
if(isset($_FILES['esyalistesi']['name'][0]) && $_FILES['esyalistesi']['name'][0] != "") {  
    $total_count = explode(";", $esyalistesidb);
...
$sql = "UPDATE mulkkayit SET esyalistesi='$esyalistesipaths' where id='$mulkyapino'"; 
...
else 
{
 ...
 // Dosyanın boş olup olmadığını kontrol etmek
// Dosyanın boş olup olmadığını kontrol etmek
if(isset($_FILES['ekbelgeyukle']['name'][0]) && $_FILES['ekbelgeyukle']['name'][0] != "")    
{    
    
        $total_count = explode(";", $ekbelgeyukledb);
// Loop through every file
...
$sql = "UPDATE mulkkayit SET ekbelgeyukle='$ekbelgeyuklepaths' where id='$mulkyapino'"; 
...
}else 
{
  echo "Error: " . $sql . "<br>" . $conn->error;
       ...
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkid.' numaralı mülk başarıyla güncellenmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';
