...
<?php 
    ...
    if($_FILES['kaporafat']['name']!=NULL) {
        $kaporafat = $_FILES['kaporafat'];
    }
    else
    {
      Notdevam();
...
$kaporafatpaths="";
    ...
    $basePath = "proje/mulk/{$projectId}/kapora/fat/";
createDirectory($basePath);//KLASÖR İLK KEZ BURDA OLUŞTURULDU        
      
$i=randomNumbersInRange(1,99999);
       // Dosya adını ve uzantısını al
        $originalFileName = $_FILES['kaporafat']['name'];
        $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
$newFilePath = $basePath.$projectId."_kirafat_".$currentDate.$i. "." . $fileExtension;
//DBye yazmak için değişkene dosya pathını kopyala
$kaporafatpaths=$kaporafatpaths.$newFilePath;
         if(move_uploaded_file($kaporafat['tmp_name'], $newFilePath)) {
        //echo "OK2"; 
...
//echo "Connected successfully";
      ...
      $sql = "UPDATE kirakaporakayit SET kaporafatpath='$kaporafatpaths' where kirakaporakayit.kirakey='$buserid'";
 ...
 if ($conn->query($sql) === TRUE) {
     ...
     echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulknoid.' numaralı mülkün faturası başarıyla yüklenmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>'; 
