...
if ($result = $conn -> query("SELECT * FROM kiralamakayit where  kiralamakayit.company_id = '{$_SESSION['company_id']}' AND kiralamakayit.isdeleted !=1 AND kiralamakayit.kiralamakey='$kirakey'")) {
  while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {
        $kaporaid=$row['id'];
 ...
 //echo $kirakaporaeklendi; 
   ...
   echo "<br>";
   echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu kiralamaya ait tahsilat yapılamaz. Mülkün öncelikle kiraya verilmesi gerekmektedir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
}
...
<br>
    ...
    <div class="parentmain2">
        <div class="options">
            <div class="container">
                ...
                <form id="busers" action="/tahsilatyapaction.php" method="post" onsubmit="return validatecallForm()" enctype="multipart/form-data">
           ...
           echo '<input type="text" id="kirakey" name="kirakey" class="form-control" value="'.$kirakey.'" required="required" style="display:none;">';
               ...
               ?>
                    ...
                    <br><br>
                    <div class="row">
                        <input type="submit" name="submit" id="onaybutton" value="Tahsilatı Onaya Gönder">
                    </div>
                </form>
            </div>
        </div>
    ...
    if (tahsilatCount >= 6) {
        alert("En fazla 6 tahsilat eklenebilir.");
        return;
    }
...
<hr style="border: 3px solid black;">
                <div class="tahsilat-form" id="${formId}">
                    <div class="row">
                        ...
                        <div class="col-75">
                            <input type="date" id="tahsilattarihi_${formId}" name="tahsilattarihi[]" style="font-weight:bolder" min="<?php echo $kiralamatarihi; ?>" required="required">
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                        ...
                        <div class="col-75">
                            <select name="tahsilatturu[]" id="tahsilatturu_${formId}" class="form-control" required onchange="handleCurrencyChange('${formId}')">
                                ...
                                <option>KİRA</option>
                                <option>AİDAT</option>
                                <option>DEPOZİTO</option>
                                <option>KAPORA</option>
                                <option>SU</option>
                                <option>HİZMET</option>
                                <option>İNTERNET</option>
                                <option>KOMİSYON</option>
                            </select>
                        </div>
                    </div>
                    <hr>
                    ...
                    <div class="row">
    <div class="col-25">
        <label for="tahsilatnot_${formId}" style="font-weight:bolder">Özel Not (Max 500 karakter)</label>
    </div>
    <div class="col-75">
        <textarea id="tahsilatnot_${formId}" name="tahsilatnot[]" maxlength="500" oninput="validateNotePattern(this)"></textarea>
        <small id="noteError_${formId}" style="color: red; display: none;">Lütfen geçerli karakterler kullanın (harf, sayı ve belirli özel karakterler)</small>
    </div>
</div>
                    <hr>
                    <div class="row">
