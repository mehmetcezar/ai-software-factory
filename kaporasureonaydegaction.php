...
/***********************VERİ TABANI YAZMA İŞLEMLERİ*****************************/
    ...
    $tarih = date('Y-m-d');
     ...
     $sql = "INSERT INTO kaporasurereddet (id, username, date, mulkno, kirakey, yetkiliaciklama ) VALUES ('', '$uname','$date','$mulkno','$kirakaporasonkey','$aciklama')";
    
    if ($conn->query($sql) === TRUE) {
   

      $kaporasureonayinda=3;   //reddedildi
        //$kaporasureuzatmadate="0000-00-00";
        $status="KAPORASURERED";
         $sql = "UPDATE mulkkayit SET kaporasureonayinda='$kaporasureonayinda', status='$status' where id='$mulkno'"; 
     ...
     if ($conn->query($sql) === TRUE) {
            ...
            echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülke ait kapora süre uzatma süreci iptal edilmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
         }
         else 
{
  echo "Error: " . $sql . "<br>" . $conn->error;
