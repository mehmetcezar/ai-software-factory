...
<?php 
    ...
    if($_FILES['kaporasozimzali']['name']!=NULL) {
        $kaporasozimzali = $_FILES['kaporasozimzali'];
    }
    else
    {
      Notdevam();
...
$kaporasozimzalipaths="";
    
    
    $basePath = "proje/mulk/{$projectId}/kapora/imzalisoz/";
createDirectory($basePath);//KLASÖR İLK KEZ BURDA OLUŞTURULDU        
      
$i=randomNumbersInRange(1,99999);
       // Dosya adını ve uzantısını al
        $originalFileName = $_FILES['kaporasozimzali']['name'];
        $fileExtension = pathinfo($originalFileName, PATHINFO_EXTENSION);
$newFilePath = $basePath.$projectId."_satiskaporasozimzali_".$currentDate.$i. "." . $fileExtension;
//DBye yazmak için değişkene dosya pathını kopyala
$kaporasozimzalipaths=$kaporasozimzalipaths.$newFilePath;
         if(move_uploaded_file($kaporasozimzali['tmp_name'], $newFilePath)) {
        //echo "OK2"; 
...
//echo "Connected successfully";
      ...
      $sql = "UPDATE satiskaporakayit SET kaporasozimzalipath='$kaporasozimzalipaths' where satiskaporakayit.id='$buserid'";
