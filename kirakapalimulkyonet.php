...
<h1 class="top-text">KİRAYA KAPALI MÜLKLER</h1>
<!-------------------------------------------------------->
...
/****/            
	...
	//$empSQL = "SELECT * FROM labproje where labproje.ispassived !=1 AND musterikayitid='$musterikayitid'";
       $empSQL = "SELECT * FROM mulkkayit where  mulkkayit.company_id = '{$_SESSION['company_id']}' AND mulkkayit.kiralamadurumu='KAPALI'";
...
$kirakaporasontarih=$emp['kirakaporasontarih'];    
 $datetime22 = new DateTime("$kirakaporasontarih");           
  $kirakaporasontarih2 = $datetime22->format('d-m-Y');          
            ...
            if($emp['kirayaayrildi']==1){
              ...
              $bilgikaporacheck=0;  
                ...
                $color="#00E0FF";
                 $bilgikaporacheck=1;}
                if($emp['kirakaporaeklendi']!=1 && $emp['kaporaonayinda']==3){
  ...
  $result2 -> free_result();
                ...
                $tarihnot = new DateTime($noticetarihi);
                $noticetarihi = $tarih->format('d-m-Y');
...
$mulknox=$emp['id'];
  ...
  while ($row2y = $result2y -> fetch_array(MYSQLI_ASSOC)) {
      ...
      $katno=$row2y['kat'];
      $alan=$row2y['alan'];
      $siteadi=$row2y['siteadi'];
}
     
  $result2y -> free_result();
}
            if($emp['kiralamadurumu']=="KAPALI" && $emp['kirayaactarih']!="0000-00-00"){
             $kirayaactarih=$emp['kirayaactarih'];
