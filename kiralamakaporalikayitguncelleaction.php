...
<?php 
    ...
    else
    {
     ...
     if($_POST['kiralamaid']==NULL){
        Notdevam();
    }
    else
    {
      $kiralamaid=$_POST['kiralamaid'];
...
//echo "Connected successfully";
  ...
  $result -> free_result();
   ...
   if ($satildiison == 1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu mülk satılmıştır. Satılmış olan mülk güncellenmez. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
} 
    
       if ($kiraison == 1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu mülkte işlem sırasında kira kaporası silinmiştir. Kira kaporası eklendikten sonra yeni kiralama kaydı oluşturulması gerekir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
} 
  if ($kiralandiison == 1) {
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">Bu mülkün kiralama işlemi muhasebe tarafından onaylanmıştır. Değişiklik Yapılamaz. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Ana Menü</button></div></div></div></div>';
   exit();
}   
...
/*
   ...
   */ 
     ...
     /*$sql = "INSERT INTO kiralamakayit (id, date, username, yapino, mulkno, stopajvergisi, kirabedeli, kirabedeliparabirimi, kirasuresi, kiraodemebicimi, toplamkirabedeli, kaporaid, ilktaksitmiktari, depozitobedeli, depozitobedeliparabirimi, odenendepozito, depozitovadesayisi, komisyon, komisyonbedeli, komisyonbedeliparabirimi, komisyontahsili, kiralamaozelnot, company_id) VALUES ('', '$date','$uname','$yapino','$mulkno','$stopajvergisi','$kirabedeli','$kirabedeliparabirimi','$kirasuresi','$kiraodemebicimi','$toplamkirabedeli','$kaporaid','$ilktaksitmiktari','$depozitobedeli','$depozitobedeliparabirimi','$odenendepozito','$depozitovadesayisi','$komisyon','$komisyonbedeli','$komisyonbedeliparabirimi','$komisyontahsili','$kiralamaozelnot', '{$_SESSION['company_id']}')";*/
    
    $sql = "UPDATE kiralamakayit SET date = '$date', username = '$uname', yapino = '$yapino', mulkno = '$mulkno', stopajvergisi = '$stopajvergisi', kirabedeli = '$kirabedeli', kirabedeliparabirimi = '$kirabedeliparabirimi', kirasuresi = '$kirasuresi', kiraodemebicimi = '$kiraodemebicimi', toplamkirabedeli = '$toplamkirabedeli', kaporaid = '$kaporaid', ilktaksitmiktari = '$ilktaksitmiktari', depozitobedeli = '$depozitobedeli', depozitobedeliparabirimi = '$depozitobedeliparabirimi', odenendepozito = '$odenendepozito', depozitovadesayisi = '$depozitovadesayisi', komisyon = '$komisyon', komisyonbedeli = '$komisyonbedeli', komisyonbedeliparabirimi = '$komisyonbedeliparabirimi', komisyontahsili = '$komisyontahsili', kiralamaozelnot = '$kiralamaozelnot' WHERE id = $kiralamaid";
 ...
 */        
  $sql = "UPDATE kiralamakayit SET kiralamasozpath='$full_pathx' where kiralamakayit.id='$kiralamaid'";        
if ($conn->query($sql) === TRUE) {
  ...
  <button type="submit" id="autoClickBtn" class="gobackcap" style="background:green;color:white;font-size:15px;font-weight:bolder;">Kira Sözleşmesini İndir</button>
            ...
            echo "<br>";
       echo "<br>";
       echo '<div class="parentmain2"><div class="options"><div class="container"><z class="textcap">'.$mulkno.' numaralı mülkün kiralama bilgileri başarıyla güncellenmiştir. Ana menüye dönmek için tıklayınız.</z><div class="gobackcapdiv"><br><br><button class="gobackcap" type="submit" name="submit" onclick="opensession()">Devam</button></div></div></div></div>';  
